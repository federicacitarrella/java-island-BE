...
public class TransactionController {
    ...
    public ResponseEntity getTransactions(@RequestHeader("Authorization") String token, @PathVariable String accountNumber) {
        ...
        //Add try
        List<Transaction> transactions = transactionService.findAllByAccountOwnerIdAndAccountNumberFrom(jwtUtils.getAccounOwnerIdFromJwtToken(token), accountNumber);
    ...
    public ResponseEntity transfer(@RequestHeader("Authorization") String token, @RequestBody TransferRequest request) throws IOException {
        ...
        switch ((int) response) {
            case 1:
                Transaction transaction = new Transaction(request.getType(), request.getAccountNumberFrom(), request.getAccountNumberTo(),
    ...
    public ResponseEntity transaction(@RequestHeader("Authorization") String token, @RequestBody TransactionRequest request) throws IOException {
        ...
        switch ((int) response) {
            case 1:
                Transaction transaction = new Transaction(request.getType(), request.getAccountNumber(), null,
