...
public class TransactionController {
    ...
    @GetMapping
    public ResponseEntity getTransactions(@RequestHeader("Authorization") String token) { //uscite col meno lambda
        ...
        switch ((int) response) {
            case 1:
                TransactionDto transaction = new TransactionDto(request.getType(), request.getAccountNumberFrom(), request.getAccountNumberTo(),
                        jwtUtils.getAccounOwnerIdFromJwtToken(token), Instant.now(), request.getCause(), request.getAmount());
        ...
        switch ((int) response) { //Utility
            case 1:
                TransactionDto transaction = new TransactionDto(request.getType(), request.getAccountNumber(), null,
                        jwtUtils.getAccounOwnerIdFromJwtToken(token), Instant.now(), null, request.getAmount());
                transactionService.save(transaction);
