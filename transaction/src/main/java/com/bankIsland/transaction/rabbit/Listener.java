...
public class Listener {
    ...
    public String listenClosingAccount(ClosingAccountMessage message) {
        ...
        try {
            ...
            transactionService.save(new TransactionDto(1, transactionDto.getAccountNumberTo(), transactionDto.getAccountNumberFrom(), transactionDto.getAccountOwnerId(), Instant.now(), "Chiusura conto " + transactionDto.getAccountNumberTo(), transactionDto.getAmount()));
    ...
    public Boolean listenOpeningTransaction(OpeningTransactionMessage message) {
        ...
        try {
            transactionService.save(new TransactionDto(1, message.accountNumberFrom(), message.accountNumberTo(), message.accountOwnerId(), Instant.now(), "Apertura conto " + message.accountNumberTo(), message.amount()));
            return true;
        } catch (Exception e) {
            return false;
        }
