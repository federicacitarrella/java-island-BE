...
public class Listener {
    ...
    public String listenClosingAccount(ClosingAccountMessage message) {
        ...
        try {
            TransactionDto transactionDto = transactionService.findByAccountNumberTo(message.getAccountNumber());

            transactionService.save(new TransactionDto(1,transactionDto.getAccountNumberTo(),transactionDto.getAccountNumberFrom(),transactionDto.getAccountOwnerId(),new Date(),"Chiusura conto "+transactionDto.getAccountNumberTo(),transactionDto.getAmount()));
    ...
    public Boolean listenOpeningTransaction(OpeningTransactionMessage message) {
        ...
        try {
            transactionService.save(new TransactionDto(1, message.getAccountNumberFrom(), message.getAccountNumberTo(), message.getAccountOwnerId(), new Date(),"Apertura conto "+message.getAccountNumberTo(), message.getAmount()));
