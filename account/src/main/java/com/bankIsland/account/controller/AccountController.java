            ...
            if (accountOwnerId == account.getAccountOwnerId()) {
                if (request.getAmount() <= account.getBalance() && request.getAmount() > 0) {
                    ...
                    if (sender.openingTransactionRequest(message)){
                        ...
                        AccountDto newAccount = new AccountDto(accountNumber, request.getFirstName(), request.getLastName(), request.getAmount(), 2, accountOwnerId);
                        return ResponseEntity.ok(accountService.save(newAccount));
                    } else {
                        ResponseEntity.internalServerError().body(new MessageResponse("Server error, try later."));
                    }
                } else {
