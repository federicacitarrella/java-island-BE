...
public class AccountController {
            ...
            if (accountOwnerId == account.getAccountOwnerId()) {
                if (request.amount() <= account.getBalance() && request.amount() > 0) {
                    ...
                    if (sender.openingTransactionRequest(message)) {
                        ...
                        logger.warn("Unable to perform the creation transaction");
                        throw new ApiBankException("Unable to perform the creation transaction.");
                    }
        ...
        try {
            ...
            if (accountOwnerId == account.getAccountOwnerId()) {
                if (accountService.closeAccount(account)) {
                    ...
                    logger.warn("Unable to request account " + account_id + " closure");
                    return ResponseEntity.badRequest().body(new MessageResponse("Error: check that the card balance is equal to 0."));
                }
            }
    ...
    public ResponseEntity<?> getAccounts(@RequestHeader("Authorization") String token) {
        ...
        try {
            return ResponseEntity.ok(accountService.findAllByAccountOwnerId(jwtUtils.getAccountOwnerIdFromJwtToken(token)));
        } catch (NoSuchElementException e) {
    ...
    public ResponseEntity<?> validateAccount(@PathVariable int account_id) {
        ...
        try {
            if (accountService.validateAccount(account_id)){
                ...
                logger.warn("Unable to validate account " + account_id);
                return ResponseEntity.internalServerError().body(new MessageResponse("Server error, try later."));
            }
        } catch (Exception e) {
            logger.warn("Unable to validate account " + account_id);
        ...
        try {
            ...
            if (account.getStatus() == 3) {
                ...
                if(accountService.rejectAccount(account)) {
                    ...
                    logger.warn("Unable to close account " + account_id);
                    return ResponseEntity.internalServerError().body(new MessageResponse("Server error, try later."));
                }
                ...
                if (accountNumberPrev != null) {
                    ...
                    if (accountService.rejectNewAccount(account, accountPrev)) {
                        ...
                        logger.warn("Unable to reject account " + account_id);
                        return ResponseEntity.internalServerError().body(new MessageResponse("Server error, try later."));
                    }
                } else {
                    logger.warn("Unable to perform the transaction");
                    return ResponseEntity.internalServerError().body(new MessageResponse("Server error, try later."));
                }
            }
            logger.warn("Unable to reject account " + account_id);
    ...
    public ResponseEntity<?> deleteAccount(@PathVariable int account_id) {
        ...
        try {
            if (accountService.delete(accountService.findById(account_id))) {
                ...
                logger.warn("Unable to delete account " + account_id);
                return ResponseEntity.badRequest().body(new MessageResponse("Action not available for this account."));
            }
        } catch (NoSuchElementException e) {
