...
public class AccountOwnerController {
    ...
    public ResponseEntity<?> deleteUser(@RequestHeader("Authorization") String token) throws ApiBankException{
        try {
            //Evitare che l'utente possa cancellarsi da solo
            ...
            return ResponseEntity.ok(new MessageResponse("Correntista eliminato correttamente."));
        } catch (Exception e) {
            throw new ApiBankException("Errore: impossibile eliminare l'utente.");
        }
    }
