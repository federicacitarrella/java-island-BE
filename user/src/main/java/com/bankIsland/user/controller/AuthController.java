...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) throws JsonProcessingException {
        ...
        AccountOwnerDto accountOwnerDto = new AccountOwnerDto(signUpRequest.getFirstName(),
                signUpRequest.getLastName(),
                signUpRequest.getEmail(),
                signUpRequest.getBirthDate());
        accountOwnerService.save(accountOwnerDto);

        UserDto userDto = new UserDto(signUpRequest.getEmail(),
                encoder.encode(signUpRequest.getPassword()),
                new HashSet<Role>(List.of(roleRepository.findByName(ERole.C).get())),
                accountOwnerDto.getId());
        userService.save(userDto);

        boolean response = sender.send(new CreationAccountMessage(accountOwnerDto.getId(), accountOwnerDto.getFirstName(), accountOwnerDto.getLastName()), userDto.getId());

        if(response) {
            return ResponseEntity.ok(new MessageResponse("User registered successfully!"));
        } else {
            accountOwnerService.delete(accountOwnerDto);
