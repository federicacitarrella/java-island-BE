...
public class AccountOwnerServiceImpl implements AccountOwnerService {
    ...
    public AccountOwnerDto registration(SignupRequest signupRequest) {
        AccountOwnerDto accountOwnerDto = new AccountOwnerDto(signupRequest.firstName(),
                signupRequest.lastName(),
                signupRequest.email(),
                signupRequest.birthDate());
        accountOwnerDto = save(accountOwnerDto);
        return accountOwnerDto;
    }

    @Override
    public boolean delete(AccountOwnerDto accountOwnerDto) {
        try {
            AccountOwner accountOwner = new AccountOwner();
            BeanUtils.copyProperties(accountOwnerDto, accountOwner);
