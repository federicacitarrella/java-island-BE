...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public UserDto registration(SignupRequest signupRequest, int id) throws ApiBankException{
        UserDto userDto = new UserDto(signupRequest.email(),
                encoder.encode(signupRequest.password()),
                new HashSet<Role>(List.of(roleRepository.findByName(ERole.C)
                        .orElseThrow(() -> new ApiBankException("Role not found")))),
                id);
        return save(userDto);
    }
